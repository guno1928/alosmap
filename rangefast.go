...
// minRangeEntriesPerWorker bounds parallel-Range fanout by available work:
// spawning one goroutine per shard wastes thousands of ~1us spawns on a
// medium map where each worker would touch only a handful of entries. Capping
// workers to totalLive/minRangeEntriesPerWorker keeps big maps fully parallel
// while small/medium maps spawn only as many goroutines as there is work for.
const minRangeEntriesPerWorker = 2048
	...
	for s := range shards {
		...
		for ; i+8 <= nSlots; i += 8 {
			...
			for j := 0; j < 8; j++ {
				...
				if cv, ok := current.loadSimple(); ok {
					if !visitor(current.key, cv) {
						stopped.Store(true)
						return
					}
					continue
	...
	for s := range shards {
		...
		for ; i+8 <= nSlots; i += 8 {
			...
			for j := 0; j < 8; j++ {
				...
				if cv, ok := current.loadSimple(); ok {
					buf = append(buf, rangePair{key: current.key, value: cv})
					continue
				}
				v, ok := shardPtr.readEntry(current, false)
				if !ok {
