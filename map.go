...
type shard struct {
	...
	slab      atomic.Pointer[bundleSlab]
	slabDepth int
}

type bundleSlab struct {
	buf []entryBundle
	off atomic.Int64
}

func (s *shard) newBundle() *entryBundle {
	if s.slabDepth <= 0 {
		return &entryBundle{}
	}
	for {
		sl := s.slab.Load()
		if sl != nil {
			if i := sl.off.Add(1) - 1; i < int64(len(sl.buf)) {
				return &sl.buf[i]
			}
		}
		fresh := &bundleSlab{buf: make([]entryBundle, s.slabDepth)}
		fresh.off.Store(1)
		if s.slab.CompareAndSwap(sl, fresh) {
			return &fresh.buf[0]
		}
	}
}
...
// valueBox is the single per-value record. It absorbs the value's interface
// words (typ + atomically mutable data) plus optional TTL/hits/clone metadata.
// typ is immutable once published; same-type repeat stores mutate only data
// (zero allocation), so readers never see a torn typ/data pair. simple is true
// when the box carries no TTL/hits/clone metadata, letting readers take the fast
// path of reconstructing the value directly from typ/data.
type valueBox struct {
	typ  uintptr
	data atomic.Pointer[byte]
	// meta is nil for the common case (no TTL, hit limit, or clone tracking),
	// keeping a simple box at 3 words. It is allocated only for entries that
	// carry that metadata.
	meta *valueMeta
}
...
// isSimple reports that the box carries no TTL/hits/clone metadata, so readers
// can reconstruct the value directly from the interface words.
func (b *valueBox) isSimple() bool { return b.meta == nil }

func (b *valueBox) expires() int64 {
	if b.meta != nil {
		return b.meta.expiresAt
	}
	return 0
}

func (b *valueBox) hitsLoad() int64 {
	if b.meta != nil {
		return b.meta.hits.Load()
	}
	return 0
}

func (b *valueBox) cloned() int64 {
	if b.meta != nil {
		return b.meta.clonedBytes
	}
	return 0
}
...
// Prealloc enables chunked allocation of entry nodes and returns the Builder for
// chaining. Each shard allocates chunk entry nodes at once instead of one per
// insert, which lowers allocation overhead on insert-heavy maps.
//
// The default is 0 (disabled). A positive value sets the batch size: larger
// batches allocate less often but reserve more memory up front, and the memory of
// a batch is not released until every node in it is gone.
//
// Example, batch 256 nodes:
//
//	m := alosmap.NewBuilder().Capacity(100_000).Prealloc(256).Build()
//
// Example, a larger batch for a long-lived, insert-heavy map:
//
//	m := alosmap.NewBuilder().Capacity(1_000_000).Prealloc(1024).Build()
func (b *Builder) Prealloc(chunk int) *Builder {
	b.cfg.prealloc = chunk
	return b
}
...
func (b *Builder) Build() *Map {
	return New(
		...
		WithPrealloc(b.cfg.prealloc),
	)
}

// WithPrealloc returns an Option that enables chunked allocation of entry nodes.
//
// Each shard allocates chunk entry nodes at once and hands them out as keys are
// inserted. The default is 0 (disabled); positive values set the batch size.
//
// Example, batch 256 nodes:
//
//	m := alosmap.New(alosmap.WithCapacity(100_000), alosmap.WithPrealloc(256))
//
// Example, a larger batch for an insert-heavy map:
//
//	m := alosmap.New(alosmap.WithPrealloc(1024))
func WithPrealloc(chunk int) Option {
...
func New(options ...Option) *Map {
	...
	for index := range instance.shards {
		...
		currentShard.slabDepth = cfg.prealloc
...
func (m *Map) Store(key Key, value any) {
	...
	// Fast probe path: when the map has no custom cloner and there is already a
	// live simple entry whose published type matches the new value's type, we
	// publish the new value by mutating the box's data word in place (zero
	// allocation; typ is immutable so readers never see a torn pair).
	if m.noClone {
		...
		if c == fp {
			...
			if current != nil && current.hash == hash && keysEqual(current.key, key) {
				...
				if existing != nil && existing.expires() == 0 && existing.hitsLoad() >= 0 {
					var boxed *valueBox
					if m.noClone {
						boxed = newSimpleBox(value)
					} else {
						cloned, tracked := m.cloneValue(value)
						boxed = newClonedBox(cloned, tracked)
					}
...
func (m *Map) Range(visitor func(key Key, value any) bool) {
	...
	if maxByWork := int(totalLive / minRangeEntriesPerWorker); workers > maxByWork {
		workers = maxByWork
	}
	if workers < 1 {
		workers = 1
...
// newSimpleBox builds a box for a value with no TTL/hits/clone metadata. The
// value lives in the box's interface words (meta stays nil) so readers take the
// fast path.
func newSimpleBox(value any) *valueBox {
	boxed := &valueBox{}
	boxed.setVal(value)
	return boxed
}

// newClonedBox builds a box for an already-cloned value, tracking bytes only
// when nonzero (otherwise the box stays simple).
func newClonedBox(value any, trackedBytes int64) *valueBox {
	boxed := &valueBox{}
	boxed.setVal(value)
	if trackedBytes != 0 {
		boxed.meta = &valueMeta{clonedBytes: trackedBytes}
	}
	return boxed
}
		...
		for probes := 0; probes < len(currentTable.slots); probes++ {
			...
			if current.hash == hash && keysEqual(current.key, key) {
				...
				for {
					...
					if existing.expires() > 0 && time.Now().UnixNano() >= existing.expires() {
	...
	for {
		...
		for {
			...
			if current.value.CompareAndSwap(existing, nil) {
				...
				s.tombstones.Add(1)
				s.keyBytes.Add(-keySize(current.key))
				s.valueBytes.Add(-existing.cloned())
				s.needsCleanup.Store(true)
				resizeNeeded := s.shouldResizeWithTable(currentTable)
				s.endWrite()
				if resizeNeeded {
...
func (s *shard) resizeAt(minLiveEntries int, now int64, force bool) {
	...
	for index := range currentTable.slots {
		...
		if boxed.expires() > 0 && now >= boxed.expires() {
			continue
		}
		if hits := boxed.hitsLoad(); hits < 0 {
			continue
		}
...
func (s *shard) shouldResizeWithTable(currentTable *table) bool {
	...
	// Tombstone compaction is deliberately NOT triggered synchronously here.
	// Reviving stores reuse tombstoned slots, so eager per-op compaction under
	// delete/store churn causes a resize storm (every goroutine serializes on
	// the resize lock). Tombstones are reclaimed by growth resizes and by the
	// background cleaner (which checks s.tombstones), keeping deletes lock-free.
...
func (b *valueBox) requiresCleanup() bool {
	return b.meta != nil && (b.meta.expiresAt > 0 || b.meta.hits.Load() != 0)
}

// consumeHit decrements a hit-limited box's counter. It is only called for boxes
// that carry hit metadata, so b.meta is non-nil.
func (b *valueBox) consumeHit() (consumed bool, exhausted bool) {
	for {
		hits := b.meta.hits.Load()
		switch {
		case hits == 0:
			return true, false
		case hits < 0:
		...
		default:
			...
			if b.meta.hits.CompareAndSwap(hits, next) {
				return true, next < 0
			}
		}
	}
